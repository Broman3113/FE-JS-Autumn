...
window.addEventListener("DOMContentLoaded", () => {
  ...
  const checkBtn = btn => {
    switch (btn) {
      ...
      case "%":
        if (eq.innerHTML || btn === "-") {
          if (eq.innerHTML.slice(-1) !== "+" && eq.innerHTML.slice(-1) !== "-" && eq.innerHTML.slice(-1) !== "*" && eq.innerHTML.slice(-1) !== "/" && eq.innerHTML.slice(-1) !== "%") {
            // Check if the last character not equals + - * / %
            eq.innerHTML += btn;
      ...
      default:
        // The remaining values that don't need additional processing
        eq.innerHTML += btn;
    }
